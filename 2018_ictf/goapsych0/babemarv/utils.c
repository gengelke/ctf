...
void demangle(unsigned char* input_buf, unsigned char* output_buf)
{
    unsigned char pad0[32] =
      {0xeb, 0x88, 0x27, 0x41, 0x90, 0x1e, 0x33, 0x51, 0x98, 0x2b,
       0x13, 0x57, 0x18, 0x32, 0x15, 0x17, 0x23, 0x16, 0x41, 0x35,
    ...
    for (int i = 0; i < 32; ++i) {
      int companion = (i + 1) % 32;
      tmp[i] ^= pad0[pad1[i] % 32];
      tmp[i] = (input_buf[i] & 0xf) ^ (input_buf[companion] & 0xf0);
      //printf("tmp[%.2d]=%.2x\n", i, tmp[i]);

      tmp[i + 32] -= 0xcf;
      tmp[i + 32] ^= pad0[pad1[(i + 1) % 32] % 32];
      tmp[i + 32] = (input_buf[i] & 0xc) | (input_buf[companion] & 0xc0);
      //printf("tmp[%.2d]=%.2x\n", i+32, tmp[i+32]);

      tmp[i + 64] ^= pad0[pad1[(i + 2) % 32] % 32];
      tmp[i + 64] = (input_buf[i] & 0x83) ^ (input_buf[companion] & 0x15);

      tmp[i + 96] ^= pad0[pad1[(i + 3) % 32] % 32];
      tmp[i + 96] = (input_buf[i] & 0x0f) | (input_buf[companion] & 0x40);

      tmp[i + 128] ^= pad0[pad1[(i + 4) % 32] % 32];
      tmp[i + 128] = (input_buf[i] & 0xcc) ^ (input_buf[companion] & 0x01);

      tmp[i + 160] ^= pad0[pad1[(i + 5) % 32] % 32];
      tmp[i + 160] = (input_buf[i] & 0xd0) | (input_buf[companion] & 0x0d);

      tmp[i + 192] ^= pad0[pad1[(i + 6) % 32] % 32];
      tmp[i + 192] = (input_buf[i] & 0x71) ^ (input_buf[companion] & 0x2c);

      tmp[i + 224] ^= pad0[pad1[(i + 7) % 32] % 32];
      tmp[i + 224] -= pad0[pad1[(i + 12) % 32] % 32];
      tmp[i + 224] = (input_buf[i] & 0xf0) | (input_buf[companion] & 0xf);
      //printf("tmp[%.2d]=%.2x\n", i+224, tmp[i+224]);
    }


    // XOR and unshift(substract)
    tmp[0] ^= pad0[0];
    tmp[0] -= pad0[0];
    tmp[1] ^= pad0[1];
    tmp[1] -= pad0[1];
    ...
    tmp[31] -= pad0[31];
    /* for (int x=0; x<32; x++) { */
    /*   printf("tmp out [%.2d]=%.2x\n", x, tmp[x]); */
    /* } */

    memcpy(output_buf, tmp, 32);
}
